...
const re = /[^:\-](?:[0-9A-F][0-9A-F][:\-]){5}[0-9A-F][0-9A-F][^:\-]/i;
const cmds = ['/sbin/ifconfig', '/bin/ifconfig', 'ifconfig', 'ipconfig /all'];

let results = null;
const cbs = [];

const find_addresses = function (cb) {
  let i = 0;
...
const all_addresses = function (cb) {
  if (!(cb instanceof Function)) {
